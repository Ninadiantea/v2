...
# DIANASTORE PROXY - V2 Installer (Fixed Version with WebSocket Support and Status Indicators)
# Author: AI Assistant
# Version: 3.3 - Fixed WebSocket Implementation, Version Compatibility, and UI Improvements
...
cat > package.json << 'EOF'
{
  ...
  "version": "3.3.0",
  "description": "DIANASTORE PROXY Server V2 with WebSocket Support and Status Indicators",
  "main": "server.js",
  "scripts": {
  ...
  "keywords": ["proxy", "vless", "trojan", "vmess", "shadowsocks", "websocket"],
...
echo -e "${BLUE}📄 Creating server.js with WebSocket support, version fix, and status indicators...${NC}"
cat > server.js << 'EOF'
...
async function fetchProxyList() {
    try {
        ...
        for (const source of proxySources) {
            try {
                // Set a custom User-Agent and disable HTTP/2 to avoid version issues
